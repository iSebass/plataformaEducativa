...
import Phase3Workspace from './Phase3Workspace';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-screen theme-transition">
      <AnimatePresence mode="wait">
        {activePhase === null ? (
            ...
            <div className="min-h-[400px] pt-2">
              {activeDashboardTab === 'roadmap' ? (
                ...
                >
                  ...
                  <motion.div
                    variants={itemVariants}
                    className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 flex flex-col justify-between hover:border-emerald-500/55 hover:shadow-lg dark:hover:shadow-none hover:shadow-emerald-600/5 transition-all group relative overflow-hidden"
                  >
                    {/* Glow effect for Phase 3 */}
                    <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/5 rounded-full blur-2xl pointer-events-none" />

                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded font-mono font-bold text-[10px] uppercase">
                          FASE 3
                        </span>
                        <span className="text-[10px] font-extrabold px-1.5 py-0.5 bg-emerald-500 text-white rounded uppercase tracking-wider animate-pulse">
                          Nuevo 🌿
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        <h3 className="font-extrabold text-slate-800 dark:text-slate-150 text-lg group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                          Funciones y Ecuaciones
                        </h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-light">
                          Analiza la función logarítmica (dominio, gráfica, asíntota) y resuelve ecuaciones logarítmicas de todos los tipos con verificación.
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => setActivePhase('fase-3')}
                      className="mt-6 w-full inline-flex items-center justify-center gap-2 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 hover:bg-emerald-600 dark:hover:bg-emerald-600 hover:text-white dark:hover:text-white dark:text-slate-300 font-bold rounded-xl text-xs transition-all cursor-pointer"
                    >
                      Ingresar a la Fase 3
                      <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </motion.div>
                  ...
                  <motion.div
                    variants={itemVariants}
                    className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 flex flex-col justify-between hover:border-orange-500/55 hover:shadow-lg dark:hover:shadow-none hover:shadow-orange-600/5 transition-all group relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/5 rounded-full blur-2xl pointer-events-none" />

                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="px-2 py-0.5 bg-orange-500/10 text-orange-600 dark:text-orange-400 border border-orange-500/20 rounded font-mono font-bold text-[10px] uppercase">
                          FASE 4
                        </span>
                        <span className="text-[10px] font-extrabold px-1.5 py-0.5 bg-orange-500 text-white rounded uppercase tracking-wider animate-pulse">
                          Nuevo 🔥
                        </span>
                      </div>

                      <div className="p-3 bg-orange-500/10 text-orange-600 dark:text-orange-400 rounded-xl w-fit">
                        <Zap className="w-6 h-6" />
                      </div>

                      <div className="space-y-1">
                        <h3 className="font-extrabold text-slate-800 dark:text-slate-150 text-lg group-hover:text-orange-700 dark:group-hover:text-orange-400 transition-colors">
                          Ecuaciones Exponenciales
                        </h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-light">
                          Usa el logaritmo como herramienta para resolver ecuaciones exponenciales de todo tipo, desde el reconocimiento hasta cuadráticas exponenciales.
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => setActivePhase('fase-4')}
                      className="mt-6 w-full inline-flex items-center justify-center gap-2 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 hover:bg-orange-500 dark:hover:bg-orange-500 hover:text-white dark:hover:text-white dark:text-slate-300 font-bold rounded-xl text-xs transition-all cursor-pointer"
                    >
                      Ingresar a la Fase 4
                      <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </motion.div>

                  {/* Phase 5 Card (Coming Soon) */}
                  <motion.div
                    variants={itemVariants}
          ...
          <motion.div
            key="fase-3-workspace"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -20 }}
            transition={{ duration: 0.25 }}
          >
            <Phase3Workspace onBack={() => setActivePhase(null)} />
          </motion.div>
        ) : (
          <motion.div
            key="fase-4-workspace"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -20 }}
            transition={{ duration: 0.25 }}
          >
            <Phase4Workspace onBack={() => setActivePhase(null)} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
